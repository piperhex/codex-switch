import { CirclePlus, Image as ImageIcon, Plus, X } from 'lucide-react';
...
export function ChatAddButton({ disabled, onClick }: { disabled: boolean; onClick: () => void }) {
  return <button type="button" className="chat-add-button" aria-label="添加图片" disabled={disabled}
    onClick={onClick}><CirclePlus size={30} strokeWidth={1.5} /></button>;
}
