...
export class ChatController {
  ...
  private readonly connection: MobileChatConnection;
  ...
  constructor(session: AuthSession, deviceId: string) {
    this.connection = new MobileChatConnection({ session, deviceId,
  ...
  async list(options: { search?: string; archived?: boolean; more?: boolean } = {}) {
    ...
    try {
      ...
      this.update({ threads: [...new Map(threads.map((thread) => [thread.id, thread])).values()], cursor: result.nextCursor });
