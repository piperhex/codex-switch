...
export class MobileChatConnection {
  ...
  private async connect() {
    ...
    try {
      await fetchUserProfile(this.options.session);
      if (!this.active || generation !== this.generation) return;
      const keys = keyPair(getRandomBytes);
      const socket = new WebSocket(chatSocketUrl(this.options.session.baseUrl));
      this.socket = socket;
      socket.onopen = () => socket.send(JSON.stringify({ type: 'authenticate', role: 'mobile',
        accessToken: this.options.session.accessToken, deviceId: this.options.deviceId, publicKey: keys.publicKey }));
  ...
  private paired(input: { id: string; iceServers: IceServer[]; keys: ReturnType<typeof keyPair> }) {
    ...
    this.link = new ChatLink({
      ...
      // Native WebRTC exposes the same standard methods/events used by RtcPeer, with independent TS declarations.
      createPeer: (options) => new RtcPeer(options, () => (
        new NativePeerConnection({ iceServers: options.iceServers }) as unknown as RTCPeerConnection)),
