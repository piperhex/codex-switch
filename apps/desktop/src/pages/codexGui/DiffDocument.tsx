...
function DiffContent({ file, split }: { file: DiffFile; split: boolean }) {
  ...
  return <>
    <div className={styles.fileToolbar}>
      ...
      <CopyButton text={file.raw} label={t("复制 diff")} />
    </div>
    {total ? <div className={styles.codeScroll} tabIndex={0} aria-label={t("{path} 的代码差异", { path: file.path })}>
      {split ? <div className={styles.split}>
        <div className={styles.splitLabels}><span>{t("修改前")}</span><span>{t("修改后")}</span></div>
    ...
    </div> : <p className={styles.notice}>{t(file.kind === "add" ? "新增空文件" : "此文件没有可显示的文本差异。")}</p>}
    {limit < total && <button className={styles.more} onClick={() => setLimit((value) => value + PAGE_LINES)}>
      {t("继续显示（还有 {count} 行）", { count: total - limit })}</button>}
  </>;
}
...
const FileCard = memo(function FileCard({ file, split, initialOpen }: {
  ...
  return <section className={styles.file}>
    <button className={styles.fileHeader} aria-expanded={open} onClick={() => setOpen(!open)}>
      ...
      <span className={styles.kind}>{t(label)}</span><Counts added={file.added} removed={file.removed} />
...
export const DiffDocument = memo(function DiffDocument({ files: allFiles, title = "文件修改", status,
  initialOpen = false, continuous = false, filePath }: DiffDocumentProps) {
  const t = useDiffText();
  ...
  return <div className={`${styles.diff} ${continuous ? styles.continuous : ""}`}>
    <div className={styles.toolbar}><strong>{t(title)}</strong><span>{t("{count} 个文件", { count: fileCount })}</span>
      <Counts added={added} removed={removed} />
      {status && <span role="status">{t(status)}</span>}
      <div className={styles.modes} role="group" aria-label={t("差异显示方式")}>
        <button aria-pressed={!split} onClick={() => setSplit(false)}>{t("统一")}</button>
        <button aria-pressed={split} onClick={() => setSplit(true)}>{t("并排")}</button>
      </div>
    </div>
