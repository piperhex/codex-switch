...
export function DetailsWorkspace({ selected, active, children, enabled = true, className = '', contentClassName = '' }: {
  ...
  const visible = Boolean(entry && !minimized && active && enabled);
  ...
  useEffect(() => { setEntry(null); setMinimized(false); setExpanded(false); }, [selected, enabled]);
  ...
  return <DetailsContext.Provider value={enabled ? context : null}>
    <div ref={host} className={`${styles.host} ${className}`} data-dragging={resize.dragging || undefined}
      ...
      <div className={`${styles.conversation} ${contentClassName}`} style={{ marginRight: docked ? width : 0 }}>
        {children}</div>
      {entry && <aside hidden={!visible} id={panelId} aria-label={t("文件更改详情")} className={styles.drawer}
        ...
        <header className={styles.header}>
          <span className={styles.tab}><FileDiff size={16} />{t(entry.title)}</span>
          <div className={styles.controls}>
            <button aria-label={t(expanded ? "还原抽屉宽度" : "展开详情抽屉")} onClick={() => setExpanded(!expanded)}>
              {expanded ? <Minimize2 size={16} /> : <Maximize2 size={16} />}</button>
            <button aria-label={t("最小化详情抽屉")} onClick={() => { setMinimized(true); opener.current?.focus(); }}>
              <Minus size={16} /></button>
            <button ref={closeButton} aria-label={t("关闭详情抽屉")} onClick={close}><X size={17} /></button>
          </div>
        </header>
      ...
      {entry && minimized && active && enabled && <button className={styles.restore} onClick={() => setMinimized(false)}>
        <PanelRightOpen size={16} />{t("恢复文件更改")}</button>}
    </div>
