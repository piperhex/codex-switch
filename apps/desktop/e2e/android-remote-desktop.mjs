...
try {
  ...
  await input(0, 'http://127.0.0.1:1498');
  await input(1, 'mobile-test@example.test'); await input(2, 'local-test');
  ...
  await check('02-mouse-buttons', async () => {
    ...
    await waitFor(async () => (await serverState()).desktop.inputs.length >= 5, 'mouse input');
  ...
  await check('03-landscape', async () => { await tap('旋转'); await waitText('显示桌面'); });
