...
网页版默认只监听 `127.0.0.1`，也可在设置中开启局域网监听。
它既可附加在正在运行的桌面客户端中，也可通过 `--headless --port` 无界面启动。
桌面端仍负责托盘、悬浮球和其他系统集成功能。
...
- 开启云同步会把完整账号凭据和 Provider Key 上传到你配置的后端，移动端还会接收短期 Codex access token；请使用 HTTPS，并确保你信任该手机、服务器及其数据库和备份管理。
...
| Android | 常用 ABI | APK | 移动伴侣端，需要自部署后端 |
...
在已安装客户端的命令行中运行 `codex-switch.exe --headless --port=18080`，
然后访问 `http://127.0.0.1:18080`。如果桌面客户端已经运行，这条命令会为当前实例附加启动网页版；
单独无界面运行时不会创建主窗口、托盘或悬浮球。网页版默认只监听本机，
如需局域网访问可先在设置中开启“监听局域网”；`--headless` 必须和 `--port` 一起使用。
