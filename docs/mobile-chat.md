...
手机网页版和 Android App 的聊天右上角提供远程终端入口。点击后从底部展开占屏幕高度 90% 的抽屉，
在当前电脑的项目目录中打开终端，支持输入命令、查看输出，以及 Ctrl+C、Tab 和方向键。
收起抽屉保留终端会话和聊天草稿；关闭终端结束会话，切换电脑会清理旧电脑的终端。
键盘弹出时终端随可见区域调整大小。Android 的终端显示资源随 App 打包，无需下载网页资源。
...
原生 App 会按云端账号记住上次选择的电脑。再次打开时优先使用该电脑；若它离线或已移除，
则自动选择一台在线电脑。没有在线电脑时仍可查看离线缓存，并保留上次选择。
...
5. P2P 明确断开时立即选择可用中转；连接仍显示打开但不再收发时，通过每秒一次的探测在约 3 秒内识别。
   手机或 Web 发起的每轮 WebRTC 协商默认最多保留 45 秒，期间中转照常可用；明确失败时默认重试间隔至少 10 秒。
   已建立的直连短暂失去响应后，默认保留原连接约 10 秒等待恢复，持续不可用才重建。
   直连连续稳定约 3 秒后切回，避免网络波动导致反复切换。
6. 云端 WebSocket 中断不会销毁仍可用的 P2P。双方重新鉴权后，以会话恢复凭据重新挂接中转；
   admin 重启丢失内存注册表后，也可由仍运行的两端恢复路由。聊天进程、请求和历史不因此重新初始化。

管理端“聊天设置 → P2P 连接”可自定义上述三项时间，单位为秒，填写正整数，不设业务上限。
保存成功后，admin-go 立即通过 `/device-chat` WebSocket 推送配置；正在进行的协商和恢复也使用新值。
客户端重连时重新获取最新配置，旧配置缺少这些字段时使用默认值。配置保存在现有聊天设置 JSON 中，无需数据库升级。
