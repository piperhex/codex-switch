...
- 上游返回 429 时按 1、3、5 秒逐渐增加等待时间并自动重试，默认最多持续 5 分钟；达到时限后才向客户端返回最后一次 429。
...
全部账号的全局自动刷新、当前账号的独立刷新、Codex Home 自定义路径、
...
可在设置页为 Codex Switch 选择自定义 Codex Home。未配置时会优先读取 `CODEX_HOME` 环境变量，
否则使用 `~/.codex`；“使用默认位置”也按此顺序恢复。该设置不会修改系统环境变量。受管账号副本、
